#!/usr/bin/env bash
# =============================================================================
# hyprbars-status-notify
#
# Surface hyprbars plugin health to the user as desktop notifications.
...
#
# Quick early-exits:
#   * user has hyprbars disabled in config (no `plugin = ... hyprbars.so`
#     uncommented anywhere) -> exit silently. Don't talk about a feature
#     they're not using.
#
# Reads /var/lib/hyprbars/status (root-written by rebuild.sh) and tracks
# what was last shown via $XDG_CACHE_HOME/hyprbars/last-shown so the same
# notification doesn't fire on every login.
#
...
STATUS_FILE=/var/lib/hyprbars/status
SEEN_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/hyprbars"
SEEN_FILE="$SEEN_DIR/last-shown"

# ----- predicates -----

# Does the user actually want hyprbars active? Per the dots-hyprland
# convention, the load directive lives in custom/general.conf and is
# commented out by default — uncommented when the user opts in (or via
# the Settings UI toggle, which does the same edit). If commented (or
# the file is absent), don't nag.
is_hyprbars_intended() {
    local conf="${XDG_CONFIG_HOME:-$HOME/.config}/hypr/custom/general.conf"
    [[ -f "$conf" ]] || return 1
    grep -qE '^[[:space:]]*plugin[[:space:]]*=.*hyprbars\.so' "$conf"
}

# Is hyprbars currently loaded in the running compositor? Decisive signal:
# a build failure that hasn't yet caused the .so to unload (because the
# user is still in their pre-upgrade Hyprland and the .so on disk still
# matches what's in memory) is not user-visible, so don't surface it.
is_hyprbars_loaded() {
...
# Have we already notified the user that title bars are off for THIS exact
...
# title bars were off. Otherwise a recovery notification appears out of
...
case "$state" in
    failed)
        if is_hyprbars_loaded; then
            # Title bars still functional in-session despite the build
            ...
            # surface this if/when title bars actually disappear (next
            # session restart with a still-broken build).
            exit 0
        fi
        ...
        notify low \
            "Title bars are off for now" \
            "A recent system update means the title bar feature needs to refresh before it can come back. This happens automatically in the background — usually within a day, sometimes a bit longer. You don't need to do anything; title bars will reappear on their own next time you log in."
    ...
    recovered)
        ...
        notify low \
            "Title bars are back" \
            "The title bar feature finished updating and is ready to go again."
        clear_seen
        ;;
    *)
